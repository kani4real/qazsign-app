...
class CollectProcessor:
    def __init__(self):
        # model_complexity=0 -> жеңіл ("lite") модель, тегін Streamlit
        # Cloud CPU-ы үшін бұл маңызды — толық модельге қарағанда
        # 2-3 есе жылдам жұмыс істейді.
        self.hands = mp_hands.Hands(max_num_hands=1, model_complexity=0, min_detection_confidence=0.7)
        self.landmarks = None
...
class RecognizeProcessor:
    def __init__(self):
        # model_complexity=0 -> "lite" модель, ауыр (1) моделге қарағанда
        # 2-3 есе жылдам жұмыс істейді, тегін Streamlit Cloud CPU-ы үшін
        ...
        self.last_conf = 0.0
        self.current_text = "..."
        # Кадр санауыштың орнына УАҚЫТ бойынша шектейміз — камера
    ...
    def recv(self, frame):
        # Кадрды кішірейтіп аламыз — MediaPipe-ге неғұрлым аз пиксель
        # берсек, соғұрлым ол жылдам жұмыс істейді. Экранға шығатын
        # видео өлшемі өзгермейді, тек танушы моделге кіретін көшірме
        # кішірейеді.
        img = frame.to_ndarray(format="bgr24")
        img = cv2.flip(img, 1)
        ...
        now = time.time()
        if self.model is not None and (now - self.last_infer_time) >= self.infer_interval:
        ...
        # Экрандағы видео әрдайым толық өлшемде, тегіс көрсетіледі —
        # тек тану есептеуі ғана сирек орындалады, сурет емес.
...
with tab1:
    ...
    if uploaded is not None:
        df_up = load_csv_data(uploaded)  # @st.cache_data арқылы қайта оқылмайды
